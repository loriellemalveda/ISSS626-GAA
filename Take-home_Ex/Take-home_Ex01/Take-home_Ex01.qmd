...
{r} samples\$density \<- densities_mc

{r} samples$density <- samples$density\*1000

{r} samples2 \<- samples\[order(samples\$density),\]

{r} colorRamp \<- brewer.pal(n = 7, name = "Spectral") colorRamp \<- rev(colorRamp)

{r} tmap_mode('view')\
tm_shape(diff) +\
tm_lines("black")+ tm_shape(samples2) +\
